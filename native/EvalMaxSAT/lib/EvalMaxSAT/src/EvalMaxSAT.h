...
public:
  LocalOptimizer2(Solver_cadical *solver, const WeightVector &poids,
                  const t_weight &initialWeight)
  ...
  t_weight optimize(std::vector<bool> &solution, double timeout_sec) {
    ...
    for (int id = softVarFalsified.size() - 1; id >= 0; id--) {
      ...
      if (solver->solveLimited(
              assumSatisfied,
              timeout_sec / (double)(softVarFalsified.size() + 1)) != 1) {
...
template <class SAT_SOLVER = Solver_cadical> class EvalMaxSAT {
  ...
  std::map<t_weight, std::set<int>>
      _mapWeight2Assum; // _mapWeight2Assum[weight] = set of literals with this
                        // weight
  t_weight cost = 0;
  ////// TODO : ajouter Offset_Cost avec poids négatif !!   cost (et
  /// solutionCost?)  corespondront alors au poids sur la formule actuelle, les
  /// modification de la formule apportant des modification de poids affecterons
  /// seulement Offset_Cost  (clause de poids négatif, AM1, autre ?)
  ...
  struct LitCard {
    ...
    LitCard(std::shared_ptr<CardIncremental_Lazy<EvalMaxSAT<SAT_SOLVER>>> card,
            int atMost, t_weight initialWeight)
        : card(card), atMost(atMost), initialWeight(initialWeight) {
      assert(card != nullptr);
      assert(atMost > 0);
      assert(initialWeight > 0);
    }

    int getLit() const { return card->atMost(atMost); }
  };
  std::vector<std::optional<LitCard>>
      _mapAssum2Card; // _mapAssum2Card[lit] = card associated to lit

  std::deque<std::tuple<std::vector<int>, t_weight>>
      _cardToAdd;               // cardinality to add
...
private:
  ...
  double _base = 400;
  double _minimalRefTime = 1;
  double _maximalRefTime = 5 * 60;
  TimeOut totalSolveTimeout = TimeOut(1800);
...
public:
  ...
  void setCoef(double initialCoef, double coefOnRefTime) {
    ...
    _base = (-initialCoef /
             (coefOnRefTime *
              W((-initialCoef * std::exp(-initialCoef / coefOnRefTime)) /
                coefOnRefTime)));
  }
  ...
  void unactivateDelayStrategy() { _delayStrategy = false; }
  void unactivateMultiSolveStrategy() { _multiSolveStrategy = false; }
  void unactivateUBStrategy() { _UBStrategy = false; }
  ///
  ////////////////////////
...
public:
  ...
  ~EvalMaxSAT() { delete solver; }

public:
  bool isWeighted() { return _mapWeight2Assum.size() > 1; }
  ...
  int newSoftVar(bool value, long long weight) {
    ...
    _mapAssum2Card.push_back({});
    _mapWeight2Assum[weight].insert((((int)value * 2) - 1) *
                                    ((int)(_poids.size()) - 1));
  ...
  int addClause(const std::vector<int> &clause,
                std::optional<long long> w = {}) {
    if (w.has_value() == false) { // Hard clause
      ...
      if (w.value() == 0)
        return 0;
      if (clause.size() >
          1) { // Soft clause, i.e, "hard" clause with a soft var at the end
        if (w.value() > 0) {
        ...
        assert(clause.size() == 0); // Special case: empty soft clause.
        cost += w.value();
      }
    }
    return 0;
...
private:
  std::set<int> initAssum(t_weight minWeightToConsider = 1) {
    ...
    for (auto itOnMapWeight2Assum = _mapWeight2Assum.rbegin();
         itOnMapWeight2Assum != _mapWeight2Assum.rend();
         ++itOnMapWeight2Assum) {
...
public:
  void disableOptimize() { toOptimize = false; }
  ...
  bool solve() {
    ...
    auto minWeightToConsider =
        chooseNextMinWeight(_mapWeight2Assum.rbegin()->first + 1);
      ...
      if (resultLastSolve) { // SAT
        ...
        for (;;) {
          if (resultLastSolve == 0) { // UNSAT
            double maxLastSolveOr1 = std::min<double>(
                _minimalRefTime + chronoLastSolve.tacSec(), _maximalRefTime);

            auto conflict = solver->getConflict(assum);
            MonPrint("conflict size = ", conflict.size(), " trouvé en ",
                     chronoLastSolve.tacSec(),
                     "sec, donc maxLastSolveOr1 = ", maxLastSolveOr1);
            ...
            if (_multiSolveStrategy && (conflict.size() > 3)) {
              Chrono C;
              int initialConflictSize = conflict.size();
              while ((nbSolve < 3 * lastImprove) || (nbSolve < 20) ||
                     ((C.tacSec() <
                       ((double)std::min(initialConflictSize / 166.6, 1.0)) *
                           0.1 * maxLastSolveOr1 * getTimeRefCoef()) &&
                      (nbSolve < 1000))) {
                if ((conflict.size() <= 3)) {
                  break;
                }
                ...
                auto p =
                    oneMinimize(tmp,
                                std::min(conflict.size() / 166.6, 1.0) *
                                    maxLastSolveOr1 * getTimeRefCoef() * 0.1,
                                10, true);

                if (p < bestP) {
                  bestP = p;
                  conflict = tmp;
                  MonPrint("Improve conflict at the ", nbSolve,
                           "th solve = ", conflict.size(), " (p = ", p, ")");
                  if (conflict.size() <= 2) {
                    break;
                  }
                  lastImprove = nbSolve;
                ...
                if (C.tacSec() >=
                    std::min((double)initialConflictSize / 166.6, 1.0) *
                        maxLastSolveOr1 * getTimeRefCoef()) {
                  MonPrint("Stop second solve after ", nbSolve,
                           "th itteration. (nb=", conflict.size(), "). (",
                           C.tacSec(), " > ", maxLastSolveOr1, " * ",
                           getTimeRefCoef(), ")");
                  break;
                }
                ...
                auto res = solver->solveWithTimeout(
                    vAssum, maxLastSolveOr1 * getTimeRefCoef() - C.tacSec());
                if (res == -1) {
                  MonPrint("Stop second solve because solveWithTimeout. (nb=",
                           conflict.size(), ").");
                  break;
                }
              }
              MonPrint("Stop second solve after ", nbSolve, " itteration in ",
                       C.tacSec(), " sec. (nb=", conflict.size(), ").");
            }
            ...
            MonPrint(_mapWeight2Assum.rbegin()->first, " >= ", solutionCost,
                     " - ", cost, " (", solutionCost - cost, ") ?");
            ...
            if (_litToRelax.size()) {
              ...
              if (_UBStrategy && toOptimize) {
                ...
                auto curCost = LO.optimize(
                    curSolution,
                    std::min(0.1 * chronoLastOptimize.tacSec(), 60.0));
              ...
              while (_cardToAdd.size()) {
                std::shared_ptr<CardIncremental_Lazy<EvalMaxSAT<SAT_SOLVER>>>
                    card = std::make_shared<
                        CardIncremental_Lazy<EvalMaxSAT<SAT_SOLVER>>>(
                        this, std::get<0>(_cardToAdd.front()), 1);
                ...
                if (newAssumForCard != 0) {
                  ...
                  _mapWeight2Assum[std::get<1>(_cardToAdd.front())].insert(
                      newAssumForCard);
                  _mapAssum2Card[abs(newAssumForCard)] =
                      LitCard(card, 1, std::get<1>(_cardToAdd.front()));
        ...
        for (auto c : _cardToAdd) {
          std::shared_ptr<CardIncremental_Lazy<EvalMaxSAT<SAT_SOLVER>>> card =
              std::make_shared<CardIncremental_Lazy<EvalMaxSAT<SAT_SOLVER>>>(
                  this, std::get<0>(c), 1);
          int newAssumForCard = card->atMost(1);
          if (newAssumForCard != 0) {
            assert(_poids[newAssumForCard] == 0);
...
private:
  ...
  double oneMinimize(std::vector<int> &conflict, double referenceTimeSec,
                     unsigned int B = 10, bool sort = true) {
    Chrono C;
    double minimize = 0;
    double noMinimize = 0;
  ...
  bool adapt_am1_exact() {
    ...
    for (unsigned int i = 0; i < size;) {
      ...
      std::vector<int> prop;
      // If literal in assumptions has a value that is resolvable, get array of
      // all the other literals that must have a certain value in consequence,
      // then link said literal to the opposite value of these other literals in
      // graph
      ...
      if (solver->propagate(std::vector<int>({lit1}), prop)) {
        ...
        i++;
      } else { // No solution - Remove literal from the assumptions and add its
               // opposite as a clause
        addClause({-lit1});
    ...
    for (;;) {
      ...
      if (qsize <= 2) { // Hyperparametre: Taille minimal a laquelle arreter la
                        // methode exact
        for (unsigned int i = 0; i < size; i++) {
          delete[] conn[i];
        }
        ...
        MonPrint(nbCliqueFound, " cliques found in ", (chrono.tacSec()),
                 "sec.");
        return true;
      }
      nbCliqueFound++;
  ...
  // Harden soft vars in passed clique to then unrelax them via a new
  ...
  std::vector<int> processAtMostOne(std::vector<int> clause) {
    ...
    while (clause.size() > 1) {
      ...
      assert([&]() {
        for (unsigned int i = 0; i < clause.size(); i++) {
          for (unsigned int j = i + 1; j < clause.size(); j++) {
            assert(solver->solve(std::vector<int>({clause[i], clause[j]})) ==
                   0);
          }
        }
        return true;
      ...
      MonPrint("AM1: cost = ", cost, " + ",
               w * (t_weight)(saveClause.size() - 1));
  ...
  void reduceCliqueV2(std::list<int> &clique) {
    ...
    for (auto posImpliquant = clique.begin(); posImpliquant != clique.end();
         ++posImpliquant) {
      auto posImpliquant2 = posImpliquant;
      for (++posImpliquant2; posImpliquant2 != clique.end();) {
        if (solver->solveLimited(
                std::vector<int>({-(*posImpliquant), -(*posImpliquant2)}),
                10000) != 0) { // solve != UNSAT
  ...
  unsigned int adapt_am1_FastHeuristicV7() {
    ...
    for (int VAR = 1; VAR < _poids.size(); VAR++) {
      ...
      if (solver->propagate({LIT}, prop)) {
        ...
        reduceCliqueV2(
            clique); // retirer des elements pour que clique soit une clique
  ...
  void addWeight(int lit, long long weight) {
    ...
    if (_poids[lit] == 0) {
      ...
      if (_poids[lit] > 0) {
        ...
        assert(_poids[lit] < 0 ? !_mapAssum2Card[lit].has_value()
                               : true); // If -lit becomes a soft var, it should
                                        // not be a cardinality
...
private:
  // If a soft variable is not soft anymore, we have to check if this variable
  // is a cardinality, in which case, we have to relax the cardinality.
  std::optional<int> relax(int lit) {
    ...
    if (_mapAssum2Card[var]
            .has_value()) { // If there is a cardinality constraint associated
                            // to this soft var
      int forCard =
          _mapAssum2Card[var]->card->atMost(_mapAssum2Card[var]->atMost + 1);

      if (forCard != 0) {
        if (_mapAssum2Card[abs(forCard)].has_value() == false) {
          _mapAssum2Card[abs(forCard)] = LitCard(
              _mapAssum2Card[var]->card, _mapAssum2Card[var]->atMost + 1,
              _mapAssum2Card[var]->initialWeight);
...
public:
  void setNInputVars(
      unsigned int nInputVars) { // Only used to format the size of the solution
  ...
  std::vector<bool> getSolution() {
    return std::vector<bool>(solution.begin(),
                             solution.begin() + nInputVars + 1);
    std::vector<bool> res = solution;
    res.resize(nInputVars + 1);
    return res;
  }
  t_weight getCost() { return solutionCost; }
  //
  /////////////////
};
