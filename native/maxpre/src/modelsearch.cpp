pair<uint64_t, uint64_t> Preprocessor::modelCostCheck(vector<bool>& model) {
	...
	uint64_t cost1=0;
	uint64_t cost2=0;
	if (trace.removedWeight.size()) cost2=trace.removedWeight[0];
	...
	for (unsigned c=0; c<pi.clauses.size(); ++c) {
		...
		if (!sat) {
			if (pi.clauses[c].lit.size())   cost1+=pi.clauses[c].weight(0);
			else                            cost2+=pi.clauses[c].weight(0);
		}
	}
	cost2+=cost1;
	if (cost2 < bestCost) {
